...
with open("/Users/vmisra/eclipse-workspace/MLProject/data/vmdata/sms-train.csv") as f:
    for line in f:
        curr = line.split(',')
        new_curr = [1]
        for item in curr[:len(curr)- 1]:
            new_curr.append(int(item))
        X.append(new_curr)
        Y.append([int(curr[-1])])
X = np.array(X)
...
for i in range(len(layer_2)):
    ...
    if(layer_2[i][0] == y_test[i][0]):
        correct += 1
# printing the output
print( 'total = {}'.format(len(layer_2)))
print('correct = {0}'.format(correct))
print('accuracy = {}'.format( correct * 100.0 / len(layer_2)))
